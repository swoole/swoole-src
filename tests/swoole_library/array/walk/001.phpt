--TEST--
swoole_library/array/walk: Test array_merge and array_walk
--SKIPIF--
...
function make_nested_array($depth, $breadth, $function = null, $args = [])
{
    for ($x = 0; $x < $breadth; ++$x) {
        if (null === $function) {
            $array = [0];
        } else {
            $array = [call_user_func_array($function, $args)];
        }
        for ($y = 1; $y < $depth; ++$y) {
            $array[0] = [$array[0]];
        }
