...
$pm->parentFunc = function () use ($pm) {
    go(function () use ($pm) {
        ...
        assert($ret['files']['test_jpg']['name'] === 'test.jpg');
        assert($ret['files']['test_jpg']['type'] === 'image/jpeg');
        assert(preg_match('#/tmp/swoole\.upfile\.#', $ret['files']['test_jpg']['tmp_name']));
        assert($ret['files']['test_jpg']['error'] === 0);
        assert($ret['files']['test_jpg']['size'] === filesize(TEST_IMAGE));
