...
static int swFactoryProcess_finish(swFactory *factory, swSendData *resp)
{
    ...
    if (conn->stream_fd > 0)
    {
        ...
        if (SwooleG.main_reactor->write(SwooleG.main_reactor, conn->stream_fd, (char*) &_header, sizeof(_header)) < 0)
        {
            return SW_ERR;
        }
        if (SwooleG.main_reactor->write(SwooleG.main_reactor, conn->stream_fd, resp->data, _len) < 0)
        {
            return SW_ERR;
        }
        return SW_OK;
