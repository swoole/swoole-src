...
static int swServer_start_check(swServer *serv)
{
    ...
    if (serv->task_worker_num > 0)
    {
        ...
        {
            swWarn("serv->task_worker_num == %d, Too many processes, the system will be slow. reset to %d", serv->task_worker_num, SW_CPU_NUM * SW_MAX_WORKER_NCPU);
    ...
    {
        swWarn("serv->reactor_num == %d, Too many threads, the system will be slow. reset to %d", serv->reactor_num, SW_CPU_NUM * SW_MAX_THREAD_NCPU);
    ...
    {
        swWarn("serv->worker_num == %d, Too many processes, the system will be slow. reset to %d", serv->worker_num, SW_CPU_NUM * SW_MAX_WORKER_NCPU);
