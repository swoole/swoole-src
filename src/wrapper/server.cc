...
bool Server::sendMessage(int worker_id, DataBuffer &data) {
    ...
    return swWorker_send_pipe_message(
               to_worker, &buf, sizeof(buf.info) + buf.info.len, SW_PIPE_MASTER | SW_PIPE_NONBLOCK) == SW_OK;
}
...
int Server::_onPacket(swServer *serv, swEventData *req) {
    ...
    if (packet->socket_type == SW_SOCK_UDP) {
        ...
        inet_ntop(
            AF_INET6, &packet->socket_addr.addr.inet_v6.sin6_addr, clientInfo.address, sizeof(clientInfo.address));
