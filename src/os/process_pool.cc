...
int swProcessPool_response(swProcessPool *pool, const char *data, int length) {
    if (pool->stream == nullptr || pool->stream->last_connection == nullptr ||
        pool->stream->response_buffer == nullptr) {
            ...
            if (pool->reloading == 0) {
                ...
                if (pool->reload_init == 0) {
                    ...
                    if (pool->max_wait_time) {
                        swoole_timer_add(
                            (long) (pool->max_wait_time * 1000), false, swProcessPool_kill_timeout_worker, pool);
                    }
                }
                goto _kill_worker;
            }
