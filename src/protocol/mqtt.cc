...
ssize_t swMqtt_get_package_length(swProtocol *protocol, swSocket *conn,const char *data, uint32_t size)
{
    ...
    if (recv_variable_header_size < SW_MQTT_MIN_LENGTH_SIZE)
    {//recv continue
    ...
    while (1)
    {
        ...
        mul *= 128;
        if ((byte & 128) == 0)
        {//done! there is no surplus length byte
            break;
        }
        if (variable_header_byte_count >= SW_MQTT_MAX_LENGTH_SIZE)
        {
            swoole_error_log(SW_LOG_WARNING, SW_ERROR_PACKAGE_LENGTH_TOO_LARGE, "bad request, The variable header size uper than %d", SW_MQTT_MAX_LENGTH_SIZE);
            return SW_ERR;
        }
