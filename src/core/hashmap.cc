...
int swHashMap_add_int(swHashMap *hmap, uint64_t key, void *data)
{
    ...
    node->key_int = key;
    node->data = data;
    node->key_str = nullptr;
    HASH_ADD_INT(root, key_int, node);
    return SW_OK;
}

static sw_inline swHashMap_node *swHashMap_node_find(swHashMap_node *root, const char *key_str, uint16_t key_len)
{
    swHashMap_node *out;
...
static int swHashMap_node_delete(swHashMap_node *root, swHashMap_node *del_node)
{
    ...
    if ((del_node->hh.prev == nullptr) && (del_node->hh.next == nullptr))
    {
...
{
    swHashMap_node *find, *tmp = nullptr;
    swHashMap_node *root = hmap->root;
