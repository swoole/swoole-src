...
static inline int http_client_co_open(const char *pathname, int flags, mode_t mode) {
...
static inline int http_client_co_close(int fd) {
#ifdef SW_USE_IOURING
...
static inline int http_client_co_ftruncate(int fd, off_t length) {
#if defined(SW_USE_IOURING) && defined(HAVE_IOURING_FTRUNCATE)
    return swoole_coroutine_iouring_ftruncate(fd, length);
#else
    return swoole_coroutine_ftruncate(fd, length);
#endif
}

static inline int http_client_co_lseek(int fd, off_t offset) {
#ifdef SW_USE_IOURING
...
static int http_parser_on_body(llhttp_t *parser, const char *at, size_t length) {
    ...
    if (http->download_file_name.get() && http->body->length > 0) {
        if (http->download_file == 0) {
            ...
            int fd = http_client_co_open(download_file_name, O_CREAT | O_WRONLY, 0664);
            if (sw_unlikely(fd < 0)) {
            ...
            if (http->download_offset == 0) {
                if (http_client_co_ftruncate(fd, 0) < 0) {
                    swoole_sys_warning("ftruncate(%s) failed", download_file_name);
                    return -1;
                }
            } else {
                if (http_client_co_lseek(fd, http->download_offset) < 0) {
...
void Client::reset() {
    ...
    if (download_file != 0) {
        http_client_co_close(download_file);
        download_file = 0;
