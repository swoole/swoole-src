...
static void socket_coro_register_errno_constants(int module_number, bool register_all) {
    for (const auto *error = swoole::get_errno_constants(); error->name; error++) {
        // ext-sockets owns the rest of the legacy family but does not define SOCKET_ECANCELED.
        if (!register_all && strcmp(error->name, "ECANCELED") != 0) {
            continue;
        }

        std::string name = "SOCKET_";
        name += error->name;
...
void php_swoole_socket_coro_minit(int module_number) {
    ...
    if (!zend_hash_str_find_ptr(&module_registry, ZEND_STRL("sockets"))) {
        ...
        socket_coro_register_errno_constants(module_number, true);
    } else {
        socket_coro_register_errno_constants(module_number, false);
    }
#ifdef TCP_INFO
