...
/* $Id: 267ad96f6dd19b4aa619a20655c59db669f84bd1 */
...
static const char* swoole_library_source_core_coroutine_server =
    ...
    "        $this->running = true;\n"
    "        if ($this->fn === null) {\n"
    "            $this->errCode = SWOOLE_ERRNO_EINVAL;\n"
    "            $this->running = false;\n"
    "            return false;\n"
    "        }\n"
    "        $socket = $this->socket;\n"
    ...
    "                }\n"
    "            } else {\n"
    "                if ($socket->errCode == SWOOLE_ERRNO_EMFILE || $socket->errCode == SWOOLE_ERRNO_ENFILE) {\n"
    "                    _wait:\n"
    "                    Coroutine::sleep(1);\n"
    "                    continue;\n"
    "                }\n"
    "                if ($socket->errCode == SWOOLE_ERRNO_ETIMEDOUT) {\n"
    "                    continue;\n"
    "                }\n"
    "                if ($socket->errCode == SWOOLE_ERRNO_ECANCELED) {\n"
    "                    break;\n"
    "                }\n"
    ...
    "        }\n"
    "\n"
    "        $this->running = false; // @phpstan-ignore deadCode.unreachable\n"
    "        return true;\n"
    "    }\n"
    "}\n";
...
static const char* swoole_library_source_core_coroutine_http2_multiplex_client =
    "\n"
    "/**\n"
    " * This file is part of Swoole.\n"
    " *\n"
...
static const char* swoole_library_source_core_coroutine_fast_cgi_client =
    ...
    "            if (!$recvData) {\n"
    "                if ($recvData === '') {\n"
    "                    $this->ioException(SWOOLE_ERRNO_ECONNRESET);\n"
    "                }\n"
    "                $this->ioException();\n"
    "            }\n"
    "            if (!FrameParser::hasFrame($recvData)) {\n"
    "                $this->ioException(SWOOLE_ERRNO_EPROTO);\n"
    "            }\n"
    "\n"
    "            do {\n"
...
static const char* swoole_library_source_ext_sockets =
    ...
    "    }\n"
    "    if ($length == 0) {\n"
    "        $socket->errCode = SWOOLE_ERRNO_EAGAIN;\n"
    "        return false;\n"
    "    }\n"
    "    if ($socket->type != SOCK_DGRAM) {\n"
    "        throw new RuntimeException('only supports dgram type socket');\n"
...
void php_swoole_load_library()
{
    ...
    _eval(swoole_library_source_core_coroutine_http2_multiplex_client, "@swoole/library/core/Coroutine/Http2/MultiplexClient.php");
