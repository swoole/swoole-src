...
using swoole::http2::get_setting;
...
Http2Stream::Stream(Http2Session *client, uint32_t _id) {
    ...
    ctx->keepalive = true;
    id = _id;
    local_window_size = get_setting(SW_HTTP2_SETTINGS_INIT_WINDOW_SIZE);
    remote_window_size = get_setting(SW_HTTP2_SETTINGS_INIT_WINDOW_SIZE);
}
...
bool Http2Stream::send_header(const String *body, bool end_stream) {
...
static bool http2_server_respond(HttpContext *ctx, const String *body) {
    ...
    if (ctx->co_socket || !((Server *) ctx->private_data)->send_yield || !swoole_coroutine_is_in()) {
        if (body->length > client->remote_settings.init_window_size) {
        ...
        if (tasks.size() > 1) {
            for (auto i = tasks.begin(); i != tasks.end(); i++) {
                ...
                if (!ctx->stream->send_body(
                    ...
                    client->remote_settings.init_window_size -= body->length;  // TODO: flow control?
                }
...
bool HttpContext::http2_send_file(const char *file, uint32_t l_file, off_t offset, size_t length) {
    ...
    if (body->length > 0) {
        if (!stream->send_body(body.get(), end_stream, client->local_settings.max_frame_size, offset, length)) {
            ...
            client->remote_settings.init_window_size -= length;  // TODO: flow control?
        }
    }
...
int swoole_http2_server_parse(Http2Session *client, const char *buf) {
    ...
    case SW_HTTP2_TYPE_SETTINGS: {
        ...
        while (length > 0) {
            ...
            case SW_HTTP2_SETTINGS_INIT_WINDOW_SIZE:
                client->remote_settings.init_window_size = value;
    ...
    case SW_HTTP2_TYPE_DATA: {
        ...
        // flow control
        client->local_settings.init_window_size -= length;
        stream->local_window_size -= length;

        if (length > 0) {
            if (client->local_settings.init_window_size < (get_setting(SW_HTTP2_SETTINGS_INIT_WINDOW_SIZE) / 4)) {
                http2_server_send_window_update(ctx, 0, get_setting(SW_HTTP2_SETTINGS_INIT_WINDOW_SIZE) - client->local_settings.init_window_size);
                client->local_settings.init_window_size = get_setting(SW_HTTP2_SETTINGS_INIT_WINDOW_SIZE);
            }
            if (stream->local_window_size < (get_setting(SW_HTTP2_SETTINGS_INIT_WINDOW_SIZE) / 4)) {
                http2_server_send_window_update(ctx, stream_id, get_setting(SW_HTTP2_SETTINGS_INIT_WINDOW_SIZE) - stream->local_window_size);
                stream->local_window_size = get_setting(SW_HTTP2_SETTINGS_INIT_WINDOW_SIZE);
            }
        }
