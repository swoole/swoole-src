...
Http2Stream::Stream(Http2Session *client, uint32_t _id) {
    ...
    ctx->keepalive = true;
    id = _id;
    local_window_size = get_default_setting(SW_HTTP2_SETTINGS_INIT_WINDOW_SIZE);
    remote_window_size = get_default_setting(SW_HTTP2_SETTINGS_INIT_WINDOW_SIZE);
}
...
int swoole_http2_server_parse(Http2Session *client, const char *buf) {
    ...
    case SW_HTTP2_TYPE_DATA: {
        ...
        if (length > 0) {
            if (client->local_settings.window_size < (get_default_setting(SW_HTTP2_SETTINGS_INIT_WINDOW_SIZE) / 4)) {
                http2_server_send_window_update(
                    ctx,
                    0,
                    get_default_setting(SW_HTTP2_SETTINGS_INIT_WINDOW_SIZE) - client->local_settings.window_size);
                client->local_settings.window_size = get_default_setting(SW_HTTP2_SETTINGS_INIT_WINDOW_SIZE);
            }
