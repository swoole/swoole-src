...
static int http_request_on_query_string(llhttp_t *parser, const char *at, size_t length) {
    ...
    ctx->request.path = estrndup(at, q);
    ctx->request.path_len = q;
    ...
    ZVAL_STRINGL(&tmp, (char *) (at + q + 1), length - q - 1);
    http_server_add_server_array(ht, SW_ZSTR_KNOWN(SW_ZEND_STR_QUERY_STRING), &tmp);

    // parse url params
    swoole_php_treat_data(
        PARSE_STRING,
        estrndup((char *) (at + q + 1), length - q - 1),  // it will be freed by treat_data
...
bool HttpContext::get_multipart_boundary(
    ...
    if (!http_server::parse_multipart_boundary(at, length, offset, out_boundary_str, out_boundary_len)) {
        ...
        parser.error = PHP_HTTP_INVALID_HEADER_TOKEN;
        return false;
    }
    return true;
}
