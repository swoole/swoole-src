...
namespace http2_core = swoole::http2;
...
class Client {
  public:
    ...
    http2_core::Settings local_settings = {};
    http2_core::Settings remote_settings = {};
    ...
    Client(const char *_host, size_t _host_len, int _port, bool _ssl, zval *__zobject) {
        ...
        _zobject = *__zobject;
        zobject = &_zobject;
        http2_core::init_settings(&local_settings);
    }
...
bool Client::connect() {
    ...
    client->protocol.get_package_length = http2_core::get_frame_length;
    ...
    http2_core::init_settings(&remote_settings);
...
enum swReturn_code Client::parse_frame(zval *return_value, bool pipeline_read) {
    ...
    ssize_t length = http2_core::get_length(buf);
    ...
    case SW_HTTP2_TYPE_SETTINGS: {
        ...
        http2_core::set_frame_header(frame, SW_HTTP2_TYPE_SETTINGS, 0, SW_HTTP2_FLAG_ACK, stream_id);
...
bool Client::send_window_update(int stream_id, uint32_t size) {
    ...
    *(uint32_t *) ((char *) frame + SW_HTTP2_FRAME_HEADER_SIZE) = htonl(size);
    http2_core::set_frame_header(frame, SW_HTTP2_TYPE_WINDOW_UPDATE, SW_HTTP2_WINDOW_UPDATE_SIZE, 0, stream_id);
    return send(frame, SW_HTTP2_FRAME_HEADER_SIZE + SW_HTTP2_WINDOW_UPDATE_SIZE);
}

bool Client::send_setting() {
    http2_core::Settings *settings = &local_settings;
    uint16_t id = 0;
    uint32_t value = 0;

    char frame[SW_HTTP2_FRAME_HEADER_SIZE + 18];
    memset(frame, 0, sizeof(frame));
    http2_core::set_frame_header(frame, SW_HTTP2_TYPE_SETTINGS, 18, 0, 0);
    ...
    swTraceLog(SW_TRACE_HTTP2, "[" SW_ECHO_GREEN "]\t[length=%d]", http2_core::get_type(SW_HTTP2_TYPE_SETTINGS), 18);
...
bool Client::send_ping_frame() {
    char frame[SW_HTTP2_FRAME_HEADER_SIZE + SW_HTTP2_FRAME_PING_PAYLOAD_SIZE];
    http2_core::set_frame_header(frame, SW_HTTP2_TYPE_PING, SW_HTTP2_FRAME_PING_PAYLOAD_SIZE, SW_HTTP2_FLAG_NONE, 0);
...
bool Client::send_data(uint32_t stream_id, const char *p, size_t len, int flag) {
    ...
    while (len > 0) {
        ...
        http2_core::set_frame_header(header, SW_HTTP2_TYPE_DATA, send_len, send_flag, stream_id);
...
uint32_t Client::send_request(zval *zrequest) {
    ...
    http2_core::set_frame_header(buffer, SW_HTTP2_TYPE_HEADERS, bytes, flags, stream->stream_id);

    swTraceLog(SW_TRACE_HTTP2,
               "[" SW_ECHO_GREEN ", STREAM#%d] length=%zd",
               http2_core::get_type(SW_HTTP2_TYPE_HEADERS),
               stream->stream_id,
               bytes);
    ...
    if (!is_data_empty) {
        ...
        swTraceLog(SW_TRACE_HTTP2,
                   ...
                   http2_core::get_type(SW_HTTP2_TYPE_DATA),
                   stream->stream_id,
                   len);
...
bool Client::write_data(uint32_t stream_id, zval *zdata, bool end) {
    ...
    if (ZVAL_IS_ARRAY(zdata)) {
        ...
        http2_core::set_frame_header(buffer, SW_HTTP2_TYPE_DATA, len, flag, stream_id);
        swTraceLog(SW_TRACE_HTTP2,
        ...
        http2_core::set_frame_header(buffer, SW_HTTP2_TYPE_DATA, data.len(), flag, stream_id);
        swTraceLog(SW_TRACE_HTTP2,
                   "[" SW_ECHO_GREEN ",%s STREAM#%d] length=%zu",
                   http2_core::get_type(SW_HTTP2_TYPE_DATA),
                   end ? " END," : "",
...
bool Client::send_goaway_frame(zend_long error_code, const char *debug_data, size_t debug_data_len) {
    ...
    bool ret;
    http2_core::set_frame_header(frame, SW_HTTP2_TYPE_GOAWAY, SW_HTTP2_GOAWAY_SIZE + debug_data_len, error_code, 0);
    ...
    *(uint32_t *) (frame + SW_HTTP2_FRAME_HEADER_SIZE + 4) = htonl(error_code);
    ...
    swTraceLog(SW_TRACE_HTTP2,
               ...
               http2_core::get_type(SW_HTTP2_TYPE_GOAWAY),
               last_stream_id,
               error_code);
...
static sw_inline void http2_settings_to_array(http2_core::Settings *settings, zval *zarray) {
    array_init(zarray);
