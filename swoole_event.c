...
PHP_FUNCTION(swoole_event_set)
{
    ...
    if (!SwooleG.main_reactor)
    {
        swoole_php_fatal_error(E_WARNING, "reactor is not ready, cannot swoole_event_set.");
        RETURN_FALSE;
    }
...
PHP_FUNCTION(swoole_event_del)
{
    ...
    if (!SwooleG.main_reactor)
    {
        swoole_php_fatal_error(E_WARNING, "reactor is not ready, cannot swoole_event_del.");
        RETURN_FALSE;
    }
...
PHP_FUNCTION(swoole_event_cycle)
{
    if (!SwooleG.main_reactor)
    {
        swoole_php_fatal_error(E_WARNING, "reactor is not ready, cannot swoole_event_cycle.");
        RETURN_FALSE;
    }
