...
EOF
    ...
    if test "$PHP_IOURING" = "yes" && test "$SW_OS" = "LINUX"; then
        ...
        if test $IOURING_MAJOR_VERSION > 2 || (test $IOURING_MAJOR_VERSION = 2 && test $IOURING_MINOR_VERSION >= 6); then
            if test $LINUX_MAJOR_VERSION > 6 || (test $LINUX_MAJOR_VERSION = 6 && test $LINUX_MAJOR_VERSION >= 9); then
                AC_DEFINE(HAVE_IOURING_FTRUNCATE, 1, [have io_uring ftruncate feature])
            fi
        fi
    fi
