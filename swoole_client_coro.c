...
static PHP_METHOD(swoole_client_coro, wakeup);
...
static PHP_METHOD(swoole_client_coro, on);
...
static const zend_function_entry swoole_client_coro_methods[] =
{
    ...
    PHP_ME(swoole_client_coro, wakeup, NULL, ZEND_ACC_PUBLIC)
    ...
    PHP_ME(swoole_client_coro, on, NULL, ZEND_ACC_PUBLIC)
#ifdef SWOOLE_SOCKETS_SUPPORT
...
static void php_swoole_at_shutdown(char *function)
{
#if PHP_MAJOR_VERSION < 7
    TSRMLS_FETCH_FROM_CTX(sw_thread_ctx ? sw_thread_ctx : NULL);
#endif
...
void php_swoole_check_reactor_bak()
{
...
void php_swoole_client_coro_free(zval *object, swClient *cli TSRMLS_DC)
{
...
swClient* php_swoole_client_coro_new(zval *object, char *host, int host_len, int port)
{
    ...
    zval *connection_id = sw_zend_read_property(swoole_client_coro_class_entry_ptr, object, ZEND_STRL("id"), 1 TSRMLS_CC);
...
static PHP_METHOD(swoole_client_coro, on)
{
    ...
    zval *ztype = sw_zend_read_property(swoole_client_coro_class_entry_ptr, getThis(), SW_STRL("type")-1, 0 TSRMLS_CC);
    if (ztype == NULL || ZVAL_IS_NULL(ztype))
    {
        swoole_php_fatal_error(E_ERROR, "get swoole_client_coro->type failed.");
        return;
    ...
    {
        swoole_php_fatal_error(E_WARNING, "swoole_client_coro: event callback[%s] is unknow", cb_name);
        RETURN_FALSE;
    }
    RETURN_TRUE;
}
...
PHP_FUNCTION(swoole_client_coro_select)
{
...
static int client_select_wait(zval *sock_array, fd_set *fds TSRMLS_DC)
{
    ...
    SW_HASHTABLE_FOREACH_START(Z_ARRVAL_P(sock_array), element)
        ...
        {
            swoole_php_fatal_error(E_WARNING, "object is not swoole_client_coro object[1].");
            continue;
        }
        ...
        {
            swoole_php_fatal_error(E_WARNING, "object is not swoole_client_coro object[2].");
            continue;
        }
