...
SW_API bool php_swoole_socket_set_protocol(Socket *sock, zval *zset)
{
    ...
    if (php_swoole_array_get_value(vht, "package_length_func", ztmp))
    {
        do
        {
            ...
            if (Z_TYPE_P(ztmp) == IS_STRING && (func = (swProtocol_length_function) swoole_get_function(Z_STRVAL_P(ztmp), Z_STRLEN_P(ztmp))))
            {
...
static PHP_METHOD(swoole_socket_coro, connect)
{
    ...
    if (sock->socket->get_domain() == AF_INET6 || sock->socket->get_domain() == AF_INET)
    {
        if (ZEND_NUM_ARGS() == 1)
        {
...
static PHP_METHOD(swoole_socket_coro, recvfrom)
{
    ...
    else
    {
        ...
        zval_dtor(peername);
        array_init(peername);
        if (sock->socket->get_domain() == AF_INET)
        {
        ...
        else if (sock->socket->get_domain() == AF_INET6)
        {
            add_assoc_long(peername, "port", swConnection_get_port(sock->socket->socket));
            add_assoc_string(peername, "address", (char *) swConnection_get_ip(sock->socket->socket));
        }
        else if (sock->socket->get_domain() == AF_UNIX)
        {
            add_assoc_string(peername, "address", (char *) swConnection_get_ip(sock->socket->socket));
        }
        RETURN_STR(buf);
...
static PHP_METHOD(swoole_socket_coro, getsockname)
{
    ...
    case AF_UNIX:
        ...
        break;
    default:
        php_swoole_error(E_WARNING, "unsupported address family %d for socket#%d", sock->socket->get_domain(), sock->socket->get_fd());
...
#define HANDLE_SUBCALL(res) \
    ...
    switch (optname) {
        ...
        sock->socket->set_timeout(&tv, optname == SO_RCVTIMEO ? SW_TIMEOUT_READ : SW_TIMEOUT_CONNECT | SW_TIMEOUT_WRITE);
        RETURN_TRUE;
        break;
        }
#ifdef SO_BINDTODEVICE
