...
static void SHADecode32(uint32_t *output, const unsigned char *input, unsigned int len) {
    ...
    for (i = 0, j = 0; j < len; i++, j += 4)
        output[i] = ((uint32_t) input[j + 3]) | (((uint32_t) input[j + 2]) << 8) | (((uint32_t) input[j + 1]) << 16) |
                    (((uint32_t) input[j]) << 24);
}
/* }}} */

const php_hash_ops php_hash_sha1_ops = {"sha1",
...
const php_hash_ops php_hash_sha256_ops = {"sha256",
...
const php_hash_ops php_hash_sha224_ops = {"sha224",
...
static void SHADecode64(uint64_t *output, const unsigned char *input, unsigned int len) {
    ...
    for (i = 0, j = 0; j < len; i++, j += 8)
        output[i] = ((uint64_t) input[j + 7]) | (((uint64_t) input[j + 6]) << 8) | (((uint64_t) input[j + 5]) << 16) |
                    (((uint64_t) input[j + 4]) << 24) | (((uint64_t) input[j + 3]) << 32) |
                    (((uint64_t) input[j + 2]) << 40) | (((uint64_t) input[j + 1]) << 48) |
                    (((uint64_t) input[j]) << 56);
}
/* }}} */
...
PHP_HASH_API void PHP_SHA384Update(PHP_SHA384_CTX *context, const unsigned char *input, size_t inputLen) {
    ...
    /* The cast may seem unnecessary, but on 32-bit this makes sure the result is 0 without invoking undefined
     * behaviour. */
...
const php_hash_ops php_hash_sha384_ops = {"sha384",
...
const php_hash_ops php_hash_sha512_ops = {"sha512",
...
const php_hash_ops php_hash_sha512_256_ops = {"sha512/256",
...
const php_hash_ops php_hash_sha512_224_ops = {"sha512/224",
