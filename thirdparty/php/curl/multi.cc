...
static int _php_curl_multi_setopt(php_curlm *mh, zend_long option, zval *zvalue, zval *return_value) /* {{{ */
{
	...
	switch (option) {
		...
		case CURLMOPT_PUSHFUNCTION:
			...
			error = curl_multi_setopt(mh->multi, (CURLMoption) option, _php_server_push_callback);
			if (error != CURLM_OK) {
				return 0;
			}
...
void curl_multi_register_class(const zend_function_entry *method_entries) {
	SW_INIT_CLASS_ENTRY(swoole_coroutine_curl_multi_handle,
                        ...
                        "Co\\Curl\\MultiHandle",
                        nullptr);
    SW_SET_CLASS_SERIALIZABLE(swoole_coroutine_curl_multi_handle, zend_class_serialize_deny, zend_class_unserialize_deny);
    SW_SET_CLASS_CUSTOM_OBJECT(swoole_coroutine_curl_multi_handle,
                               curl_multi_create_object,
                               curl_multi_free_obj,
                               php_curlm,
                               std);
