...
static int oci_handle_preparer(
    pdo_dbh_t *dbh, const char *sql, size_t sql_len, pdo_stmt_t *stmt, zval *driver_options) /* {{{ */
{
    pdo_oci_db_handle *H = (pdo_oci_db_handle *) dbh->driver_data;
    ...
    if (sql_len) {
        H->last_err = SW_OCIStmtPrepare(S->stmt, H->err, (text *) sql, (ub4) sql_len, OCI_NTV_SYNTAX, OCI_DEFAULT);
        if (nsql) {
            efree(nsql);
            nsql = NULL;
        }
...
static zend_long oci_handle_doer(pdo_dbh_t *dbh, const char *sql, size_t sql_len) /* {{{ */
{
    ...
    H->last_err = SW_OCIStmtPrepare(stmt, H->err, (text *) sql, (ub4) sql_len, OCI_NTV_SYNTAX, OCI_DEFAULT);
    if (H->last_err) {
        H->last_err = oci_drv_error("OCIStmtPrepare");
...
static int oci_handle_quoter(pdo_dbh_t *dbh,
                             const char *unquoted,
                             size_t unquotedlen,
                             char **quoted,
                             size_t *quotedlen,
...
static const struct pdo_dbh_methods oci_methods = {oci_handle_closer,
                                                   oci_handle_preparer,
                                                   oci_handle_doer,
                                                   oci_handle_quoter,
                                                   oci_handle_begin,
