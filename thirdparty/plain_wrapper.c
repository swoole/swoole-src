...
/* {{{ php_stream_fopen */
static php_stream *sw_php_stream_fopen_rel(const char *filename, const char *mode, zend_string **opened_path, int options STREAMS_DC)
{
...
not_relative_path:
	...
	while (ptr && *ptr) {
		end = strchr((char*)ptr, DEFAULT_DIR_SEPARATOR);
		if (end != NULL) {
			*end = '\0';
			end++;
		}
