...
ub4 _oci_error(OCIError *err, pdo_dbh_t *dbh, pdo_stmt_t *stmt, char *what, sword status, int isinit, const char *file,
			   int line) /* {{{ */
{
	...
	if (isinit) { /* Initialization error */
		...
		switch (status) {
			...
			case OCI_ERROR:
				OCIErrorGet(err, (ub4)
				1, NULL, &einfo->errcode, errbuf, (ub4)
...
static bool oci_handle_preparer(pdo_dbh_t *dbh, zend_string *sql, pdo_stmt_t *stmt, zval *driver_options) /* {{{ */
{
	...
	if (ZSTR_LEN(sql) != 0) {
		H->last_err = SW_OCIStmtPrepare(S->stmt, H->err, (text *) ZSTR_VAL(sql), (ub4)
		ZSTR_LEN(sql), OCI_NTV_SYNTAX, OCI_DEFAULT);
		if (nsql) {
...
static zend_long oci_handle_doer(pdo_dbh_t *dbh, const zend_string *sql) /* {{{ */
{
	...
	H->last_err = SW_OCIStmtPrepare(stmt, H->err, (text *) ZSTR_VAL(sql), (ub4)
	ZSTR_LEN(sql), OCI_NTV_SYNTAX, OCI_DEFAULT);
	if (H->last_err) {
		H->last_err = oci_drv_error("OCIStmtPrepare");
	...
	H->last_err = SW_OCIStmtExecute(H->svc, stmt, H->err, 1, 0, NULL, NULL,
...
static zend_string *
oci_handle_quoter(pdo_dbh_t *dbh, const zend_string *unquoted, enum pdo_param_type paramtype) /* {{{ */
{
	int qcount = 0;
	char const *cu, *l, *r;
	char *c, *quoted;
	...
	/* count single quotes */
	for (cu = ZSTR_VAL(unquoted); (cu = strchr(cu, '\'')); qcount++, cu++); /* empty loop */
...
static zend_result pdo_oci_check_liveness(pdo_dbh_t *dbh) /* {{{ */
{
	...
	OCIErrorGet(H->err, (ub4)
	1, NULL, &error_code, NULL, 0, OCI_HTYPE_ERROR);
...
static int pdo_oci_handle_factory(pdo_dbh_t *dbh, zval *driver_options) /* {{{ */
{
	...
	if (vars[0].optval) {
		...
		if (!H->charset) {
			...
			if (OCIEnvNlsCreate(&H->env, SWOOLE_PDO_OCI_INIT_MODE, 0, NULL, NULL, NULL, 0, NULL, H->charset, H->charset) != OCI_SUCCESS) {
