...
foreach ($matches[0] as $match) {
    ...
    $define_output .= "{$space4}SW_REGISTER_LONG_CONSTANT(\"{$constant_name}\", {$constant_value});\n";
}
...
$swstrerror_output = "{$space4}switch(code)\n{$space4}{\n";
foreach ($matches[0] as $match) {
    ...
    $swstrerror_output .= "{$space4}case {$match}:\n{$space8}return \"{$sw_error_str}\";\n";
}
