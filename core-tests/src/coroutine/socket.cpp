...
TEST(coroutine_socket, async_write) {
    size_t size = 1024*1024;
    String buf(size);
    buf.append_random_bytes(size);
    ...
    [&](void *arg) {
        ...
        ASSERT_EQ(retval, true);
        ASSERT_EQ(sock.errCode, 0);
        sock.async_write = true;
        sock.get_socket()->set_buffer_size(8192);
        sock.get_socket()->set_tcp_nodelay();
        auto reval = sock.send(buf.str, buf.length);
        ASSERT_EQ(reval, size);
