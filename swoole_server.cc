...
static inline zend_bool php_swoole_server_isset_callback(ServerObject *server_object, swListenPort *port, int event_type) {
...
static void server_free_object(zend_object *object) {
    ...
    swServer *serv = server_object->serv;
...
void php_swoole_server_before_start(swServer *serv, zval *zobject)
{
    ...
    for (i = 0; i < server_object->property->ports.size(); i++)
    {
        ...
        if (serv->if_require_packet_callback(port, php_swoole_server_isset_callback(server_object, port, SW_SERVER_CB_onPacket)))
        {
        ...
        if (port->open_http_protocol)
        {
            ...
            else if (port->open_http_protocol && !php_swoole_server_isset_callback(server_object, port, SW_SERVER_CB_onRequest))
            {
        ...
        {
            if (swSocket_is_stream(port->type) && !php_swoole_server_isset_callback(server_object, port, SW_SERVER_CB_onReceive))
            {
...
static int php_swoole_onTask(swServer *serv, swEventData *req)
{
    ...
    if (UNEXPECTED(!zend::function::call(server_object->property->callbacks[SW_SERVER_CB_onTask], argc, argv, &retval,
...
static PHP_METHOD(swoole_server, set)
{
    ...
    ServerObject *server_object = server_fetch_object(Z_OBJ_P(ZEND_THIS));
    sw_zend_call_method_with_1_params(server_object->property->ports.at(0), swoole_server_port_ce, nullptr, "set", nullptr,
                                      zset);
...
static PHP_METHOD(swoole_server, getCallback)
{
    ...
    ServerObject *server_object = server_fetch_object(Z_OBJ_P(ZEND_THIS));
    sw_zend_call_method_with_1_params(server_object->property->ports.at(0), swoole_server_port_ce, nullptr,
...
static PHP_METHOD(swoole_server, send)
{
    ...
    if (serv->have_dgram_sock && Z_TYPE_P(zfd) == IS_STRING && Z_STRVAL_P(zfd)[0] == '/')
    {
        ...
        ret = swSocket_sendto_blocking(
            server_socket == -1 ? server_object->property->dgram_server_socket : server_socket,
            data, length, 0,
