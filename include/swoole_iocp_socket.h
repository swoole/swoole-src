...
class IocpSocket : public Socket {
    ...
    ssize_t iocp_send(const void *_buf, size_t _n);
    ssize_t iocp_recv(void *_buf, size_t _n);
    ssize_t iocp_readv(const struct iovec *iovec, int count);
    ssize_t iocp_writev(const struct iovec *iovec, int count);
  ...
  public:
    ...
    IocpSocket *accept(double timeout = 0);
