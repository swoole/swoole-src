...
static sw_inline void sw_redis_command_var_key(INTERNAL_FUNCTION_PARAMETERS, char *cmd, int cmd_len, int min_argc, int has_timeout)
{
	...
	if (single_array)
	{
		argc = zend_hash_num_elements(Z_ARRVAL_P(z_args[0])) + 1;
	}
	else
	{
		argc++;
	...
	{
		zval *value;
		SW_HASHTABLE_FOREACH_START(Z_ARRVAL_P(z_args[0]), value)
			convert_to_string(value);
			SW_REDIS_COMMAND_ARGV_FILL(Z_STRVAL_P(value), (size_t) Z_STRLEN_P(value))
	...
	{
        if(has_timeout && Z_TYPE_P(z_args[argc-2]) != IS_LONG) {
			zend_update_property_long(swoole_redis_coro_class_entry_ptr, redis->object, ZEND_STRL("errCode"), SW_REDIS_ERR_OTHER TSRMLS_CC);
		...
		for (j = 0; j < tail; ++j)
		{
			...
			SW_REDIS_COMMAND_ARGV_FILL(Z_STRVAL_P(z_args[j]), (size_t) Z_STRLEN_P(z_args[j]))
		}
        if(has_timeout) {
			buf_len = snprintf(buf, sizeof(buf), "%ld", Z_LVAL_P(z_args[tail]));
			SW_REDIS_COMMAND_ARGV_FILL((char*)buf, buf_len);
        }
	}
    efree(z_args);
...
ZEND_BEGIN_ARG_INFO_EX(arginfo_swoole_redis_coro_connect, 0, 0, 3)
...
ZEND_BEGIN_ARG_INFO_EX(arginfo_swoole_redis_coro_setEx, 0, 0, 3)
    ZEND_ARG_INFO(0, key)
    ZEND_ARG_INFO(0, l_val)
    ZEND_ARG_INFO(0, z_value)
ZEND_END_ARG_INFO()
...
ZEND_BEGIN_ARG_INFO_EX(arginfo_swoole_redis_coro_multi, 0, 0, 1)
...
static const zend_function_entry swoole_redis_coro_methods[] =
{
    ...
    PHP_ME(swoole_redis_coro, exists, NULL, ZEND_ACC_PUBLIC)
    PHP_ME(swoole_redis_coro, type, NULL, ZEND_ACC_PUBLIC)
    PHP_ME(swoole_redis_coro, strLen, NULL, ZEND_ACC_PUBLIC)
    PHP_ME(swoole_redis_coro, lPop, NULL, ZEND_ACC_PUBLIC)
    PHP_ME(swoole_redis_coro, blPop, NULL, ZEND_ACC_PUBLIC)
    PHP_ME(swoole_redis_coro, rPop, NULL, ZEND_ACC_PUBLIC)
    ...
    PHP_ME(swoole_redis_coro, sMembers, NULL, ZEND_ACC_PUBLIC)
    ...
    PHP_ME(swoole_redis_coro, ttl, NULL, ZEND_ACC_PUBLIC)
    PHP_ME(swoole_redis_coro, pttl, NULL, ZEND_ACC_PUBLIC)
    ...
    PHP_ME(swoole_redis_coro, debug, NULL, ZEND_ACC_PUBLIC)
	...
	PHP_ME(swoole_redis_coro, bgSave, NULL, ZEND_ACC_PUBLIC)
	PHP_ME(swoole_redis_coro, lastSave, NULL, ZEND_ACC_PUBLIC)
	PHP_ME(swoole_redis_coro, flushDB, NULL, ZEND_ACC_PUBLIC)
	PHP_ME(swoole_redis_coro, flushAll, NULL, ZEND_ACC_PUBLIC)
	PHP_ME(swoole_redis_coro, dbSize, NULL, ZEND_ACC_PUBLIC)
	PHP_ME(swoole_redis_coro, bgrewriteaof, NULL, ZEND_ACC_PUBLIC)
	PHP_ME(swoole_redis_coro, time, NULL, ZEND_ACC_PUBLIC)
	PHP_ME(swoole_redis_coro, role, NULL, ZEND_ACC_PUBLIC)
    PHP_ME(swoole_redis_coro, setRange, NULL, ZEND_ACC_PUBLIC)
	PHP_ME(swoole_redis_coro, setNx, NULL, ZEND_ACC_PUBLIC)
	PHP_ME(swoole_redis_coro, getSet, NULL, ZEND_ACC_PUBLIC)
	PHP_ME(swoole_redis_coro, append, NULL, ZEND_ACC_PUBLIC)
	PHP_ME(swoole_redis_coro, lPushx, NULL, ZEND_ACC_PUBLIC)
	PHP_ME(swoole_redis_coro, lPush, NULL, ZEND_ACC_PUBLIC)
	PHP_ME(swoole_redis_coro, rPush, NULL, ZEND_ACC_PUBLIC)
	PHP_ME(swoole_redis_coro, rPushx, NULL, ZEND_ACC_PUBLIC)
	PHP_ME(swoole_redis_coro, sContains, NULL, ZEND_ACC_PUBLIC)
    PHP_MALIAS(swoole_redis_coro, sismember, sContains, NULL, ZEND_ACC_PUBLIC)
	PHP_ME(swoole_redis_coro, zScore, NULL, ZEND_ACC_PUBLIC)
	PHP_ME(swoole_redis_coro, zRank, NULL, ZEND_ACC_PUBLIC)
	PHP_ME(swoole_redis_coro, zRevRank, NULL, ZEND_ACC_PUBLIC)
	...
	PHP_ME(swoole_redis_coro, incrBy, NULL, ZEND_ACC_PUBLIC)
	PHP_ME(swoole_redis_coro, hIncrBy, NULL, ZEND_ACC_PUBLIC)
	PHP_ME(swoole_redis_coro, incr, NULL, ZEND_ACC_PUBLIC)
	PHP_ME(swoole_redis_coro, decrBy, NULL, ZEND_ACC_PUBLIC)
	PHP_ME(swoole_redis_coro, decr, NULL, ZEND_ACC_PUBLIC)
	PHP_ME(swoole_redis_coro, getBit, NULL, ZEND_ACC_PUBLIC)
	PHP_ME(swoole_redis_coro, lInsert, NULL, ZEND_ACC_PUBLIC)
	PHP_ME(swoole_redis_coro, lGet, NULL, ZEND_ACC_PUBLIC)
	PHP_MALIAS(swoole_redis_coro, lIndex, lGet, NULL, ZEND_ACC_PUBLIC)
	PHP_ME(swoole_redis_coro, setTimeout, NULL, ZEND_ACC_PUBLIC)
    PHP_MALIAS(swoole_redis_coro, expire, setTimeout, NULL, ZEND_ACC_PUBLIC)
	PHP_ME(swoole_redis_coro, pexpire, NULL, ZEND_ACC_PUBLIC)
	PHP_ME(swoole_redis_coro, expireAt, NULL, ZEND_ACC_PUBLIC)
	PHP_ME(swoole_redis_coro, pexpireAt, NULL, ZEND_ACC_PUBLIC)
	PHP_ME(swoole_redis_coro, move, NULL, ZEND_ACC_PUBLIC)
	...
	PHP_ME(swoole_redis_coro, incrByFloat, NULL, ZEND_ACC_PUBLIC)
...
{
    SWOOLE_INIT_CLASS_ENTRY(swoole_redis_coro_ce, "swoole_redis_coro", "Swoole\\Redis", swoole_redis_coro_methods);
	swoole_redis_coro_class_entry_ptr = sw_zend_register_internal_class_ex(&swoole_redis_coro_ce, swoole_client_multi_class_entry_ptr, "swoole_client_multi" TSRMLS_CC);
...
static PHP_METHOD(swoole_redis_coro, blPop)
{
	...
	{
		argc = zend_hash_num_elements(Z_ARRVAL_P(z_args[0])) + 2;
		single_array = 1;
	}
	else
	{
	...
	if (single_array)
	{
		...
		SW_REDIS_COMMAND_ARGV_FILL(Z_STRVAL_P(z_args[1]), (size_t) Z_STRLEN_P(z_args[1]))
	}
	else
	{
		int j;
...
static PHP_METHOD(swoole_redis_coro, zRangeByScore)
{
    ...
    if(z_opt && Z_TYPE_P(z_opt) == IS_ARRAY) {
		...
		if (sw_zend_hash_find(ht_opt, ZEND_STRS("limit"), (void **) &z_ele) == SUCCESS)
        {
            ...
            zval **z_off, **z_cnt;
            if(sw_zend_hash_index_find(ht_limit,0,(void**)&z_off) == SUCCESS &&
               sw_zend_hash_index_find(ht_limit,1,(void**)&z_cnt) == SUCCESS &&
...
static PHP_METHOD(swoole_redis_coro, zAdd)
{
    ...
    if (argc < 3 || Z_TYPE_P(z_args[0]) != IS_STRING) {
        efree(z_args);
		RETURN_FALSE;
    }
	SW_REDIS_COMMAND_CHECK_WITH_FREE_Z_ARGS
	...
	SW_REDIS_COMMAND_ARGV_FILL(Z_STRVAL_P(z_args[0]), (size_t)Z_STRLEN_P(z_args[0]))
	k = 1;

	if (SW_Z_TYPE_P(z_args[k]) == IS_STRING && IS_NX_XX_ARG(Z_STRVAL_P(z_args[k])))
	{
		SW_REDIS_COMMAND_ARGV_FILL(Z_STRVAL_P(z_args[k]), (size_t)Z_STRLEN_P(z_args[k]))
		k++;
		valid_params--;
	}

	if (SW_Z_TYPE_P(z_args[k]) == IS_STRING && strncasecmp(Z_STRVAL_P(z_args[k]), "CH", 2) == 0)
	{
	...
	if (SW_Z_TYPE_P(z_args[k]) == IS_STRING && strncasecmp(Z_STRVAL_P(z_args[k]), "INCR", 4) == 0)
	{
...
static PHP_METHOD(swoole_redis_coro, pSubscribe)
{
	...
	//psubscribe cannot be used in swoole_multi mode
	if (swoole_multi_is_multi_mode(getThis()) == CORO_MULTI)
	{
		zend_update_property_long(swoole_redis_coro_class_entry_ptr, getThis(), ZEND_STRL("errCode"), SW_REDIS_ERR_OTHER TSRMLS_CC);
		zend_update_property_string(swoole_redis_coro_class_entry_ptr, getThis(), ZEND_STRL("errMsg"), "psubscribe cannot be used with swoole_multi" TSRMLS_CC);
		RETURN_FALSE;
	}
...
static PHP_METHOD(swoole_redis_coro, subscribe)
{
	...
	//subscribe cannot be used in swoole_multi mode
	if (swoole_multi_is_multi_mode(getThis()) == CORO_MULTI)
	{
		zend_update_property_long(swoole_redis_coro_class_entry_ptr, getThis(), ZEND_STRL("errCode"), SW_REDIS_ERR_OTHER TSRMLS_CC);
		zend_update_property_string(swoole_redis_coro_class_entry_ptr, getThis(), ZEND_STRL("errMsg"), "subscribe cannot be used with swoole_multi" TSRMLS_CC);
		RETURN_FALSE;
	}
...
static void swoole_redis_coro_resume(void *data)
{
    ...
    if (swoole_multi_resume(redis->object, redis->result) != CORO_RESUME)
   	{
		redis->result = NULL;
   		return;
   	}
	...
	{
		sw_zval_ptr_dtor(&retval);
		//There shouldn't free redis->result, because redis has been free in __destruct
	}
	else
	{
		sw_zval_ptr_dtor(&(redis->result));
...
static void swoole_redis_coro_onResult(redisAsyncContext *c, void *r, void *privdata)
{
    ...
    if (reply == NULL)
    {
		...
		{
			sw_zval_ptr_dtor(&(redis->pipeline_result));
			redis->pipeline_result = NULL;
		}
	} else {
		...
		switch (redis->state) {
			case SWOOLE_REDIS_CORO_STATE_PIPELINE:
				if (redis->pipeline_result == NULL)
				{
					SW_MAKE_STD_ZVAL(redis->pipeline_result);
...
void swoole_redis_coro_onClose(const redisAsyncContext *c, int status)
{
    swRedisClient *redis = c->ev.data;
