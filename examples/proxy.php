...
class ProxyServer
{
    ...
    function run()
    {
        $serv = swoole_server_create("127.0.0.1", 9509, SWOOLE_BASE);
    ...
    {
		if (!isset($this->clients[$fd])) {
			swoole_server_send($this->serv, $fd, "backend server not connected. please try reconnect.");
			swoole_server_close($this->serv, $fd);
		} else {
